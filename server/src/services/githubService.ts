...
export class GithubService {
  ...
  private async fetchCommits(owner: string, repo: string): Promise<CommitData[]> {
    ...
    for (const c of raw) {
      ...
      // Heuristics for AI assistance:
      // 1. Explicit mention of AI tools (copilot, gpt, chatgpt, llm, claude, etc.)
      // 2. Co-authored commits containing 'github-copilot'
      // 3. Automated template messages for code generation
      const isAiAssisted = lowerMsg.includes('copilot') || 
                           lowerMsg.includes('gpt') || 
                           lowerMsg.includes('ai-assisted') || 
                           lowerMsg.includes('chatgpt') ||
                           lowerMsg.includes('llm') ||
                           lowerMsg.includes('claude') ||
                           lowerMsg.includes('generated by') ||
                           lowerMsg.includes('co-authored-by: github-copilot');
  ...
  async fetchRepositoryData(owner: string, repo: string): Promise<DevData[]> {
    ...
    for (const c of commits) {
      ...
      if (c.isAiAssisted) dev.aiAssistedCommits++;
